"modeHuntingApprox" <-
function (X.raw, lower = -Inf, upper = Inf, d0 = 2, m0 = 10, 
    fm = 2, crit.vals, min.int = FALSE) 
{
    ...
    for (block in 1:n.blocks) {
        ...
        for (j in seq(1, n - m + 1, by = d)) {
            k <- seq(min(n, j + minspan), min(n, (j + maxspan)), 
                by = d)
            if (length(k) > 0) {
                Tjk <- (2/(X[k] - X[j]) * (cX[k - 1] - cX[j] - 
                  X[j] * (k - j - 1)) - (k - j - 1))
                Tjks <- rbind(Tjks, cbind(j * rep(1, length(k)), 
                  k, Tjk, 1/sv[k - j - 1], cv[k - j]))
            }
        }
    }
    ...
    if (length(Dp.noadd[, 1]) > 0) {
        ...
        if (min.int == TRUE) {
            Dp.noadd <- minimalIntervals(Dp.noadd)
        }
        Dp.noadd.all <- cbind(X[Dp.noadd[, 1]], X[Dp.noadd[, 
            2]])
    }
    if (length(Dm.noadd[, 1]) > 0) {
        Dm.noadd <- Dm.noadd[order(Dm.noadd[, 1]), ]
        if (min.int == TRUE) {
            Dm.noadd <- minimalIntervals(Dm.noadd)
        }
        Dm.noadd.all <- cbind(X[Dm.noadd[, 1]], X[Dm.noadd[, 
            2]])
    }
